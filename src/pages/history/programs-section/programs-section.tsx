...
function ProgramCard({
  ...
  return (
    <div
      ref={ref}
      className={clsx(
        "rounded-fluid-md border border-white/[0.13] bg-white/[0.065] p-fluid-md transition-all duration-700 backdrop-blur-sm hover:border-white/[0.23] hover:bg-white/[0.10]",
        isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"
      )}
      style={{ transitionDelay: `${delay}ms` }}
    >
      <div className="text-4xl mb-fluid-xs">{program.icon}</div>
      <h3 className="text-fluid-lg font-bold text-white mb-fluid-xs">
        {program.title}
      </h3>
